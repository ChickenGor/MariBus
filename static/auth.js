...
async function saveUser(user,provider){await setDoc(doc(db,'users',user.uid),{uid:user.uid,email:user.email||'',displayName:user.displayName||'',photoURL:user.photoURL||'',provider,updatedAt:serverTimestamp()}, {merge:true});}
...
try{
  ...
  onAuthStateChanged(auth,user=>{if(user){elements.authCard.hidden=true;elements.accountCard.hidden=false;document.querySelector('#account-name').textContent=user.displayName||'MariBus rider';document.querySelector('#account-email').textContent=user.email||'';document.querySelector('#account-avatar').textContent=(user.displayName||user.email||'M').charAt(0).toUpperCase();}else{elements.authCard.hidden=false;elements.accountCard.hidden=true;}});
...
elements.form.addEventListener('submit',async event=>{event.preventDefault();if(!auth)return;setBusy(true);showMessage('');try{if(mode==='register'){const credential=await createUserWithEmailAndPassword(auth,elements.email.value.trim(),elements.password.value);const displayName=elements.name.value.trim();if(displayName)await updateProfile(credential.user,{displayName});await saveUser(credential.user,'password');}else{const credential=await signInWithEmailAndPassword(auth,elements.email.value.trim(),elements.password.value);await saveUser(credential.user,'password');}}catch(error){showMessage(readableError(error),'error');}finally{setBusy(false);}});
